...
function executeEventInNewWindow(eventType, trackId, eventData, isProduction = true) {
    ...
    // HTML 동적 생성 (CBT용 초록색 테마)
    const htmlContent = `<!DOCTYPE html>
<html lang="ko">
<head>
    <meta charset="UTF-8">
    <title>카카오 픽셀 이벤트 실행 - ${eventType} (CBT)</title>
...
<body>
    ...
    <script type="text/javascript">
        console.log('🔍 [CBT 새 창 이벤트] 스크립트 로딩 시작');
        
        window.onload = function() {
            console.log('🔍 [CBT 새 창 이벤트] 페이지 로드 완료');
            console.log('🔍 [CBT 새 창 이벤트] KakaoPixel 타입:', typeof kakaoPixel);
            
            setTimeout(() => {
                try {
                    console.log('🔍 [CBT 새 창 이벤트] 이벤트 실행 시작: ${eventType}');
                    
                    ${executeScript}
                    
                    document.getElementById('loading').style.display = 'none';
                    document.getElementById('result').innerHTML = 
                        '<div class="result success">✅ [CBT] ${eventType} 이벤트가 성공적으로 실행되었습니다!</div>';
                    
                    console.log('✅ [CBT 새 창 이벤트] ${eventType} 실행 완료');
                    
                    // 3초 후 창 닫기
                    setTimeout(() => {
                        console.log('🔍 [CBT 새 창 이벤트] 창 닫기');
                        window.close();
                    }, 3000);
                    
                } catch(error) {
                    console.error('❌ [CBT 새 창 이벤트] 실행 오류:', error);
                    document.getElementById('loading').style.display = 'none';
                    document.getElementById('result').innerHTML = 
...
function executeEvent() {
    ...
    // 원래 창에 상세 로그 출력
    console.log('🚀 [CBT 이벤트 실행] 새 창에서 실행됩니다.');
    console.log('📋 [CBT 실행 스크립트]', scriptLog);
    console.log('🔍 [CBT 이벤트 데이터]', {
    ...
    // 새 창에서 이벤트 실행 (CBT 환경)
    executeEventInNewWindow(data.eventType, data.trackId, eventData, true);
    
    // 기존 방식의 토스트 알림
    showResult(`[CBT] ${data.eventType} 이벤트를 새 창에서 실행합니다.`, true);
}
